...
#verificando Histograma de cada variavel, nota-se que há uma distrobuição normal, logo, testes não parametricos devem ser efetuados 
hist(dados$wt)
hist(dados$hp)
hist(dados$disp)
...
#O boxplot apresenta variabilidade, mesmo sem muitos outliers
par(mfrowc(2,2))
boxplot(dados$wt, main = "peso")
boxplot(dados$hp, main = "potência")
boxplot(dados$disp, main = "Cilindradas")
...
m2 <- stats::lm(mpg ~ wt + hp , data = dados) #removendo cilindrada do Modelo
summary(m2) #m2 o R² ajustado = 0.814 ou seja 81,4% do consumo é explicado pelas 2 variaveis peso e potencia
m3 <- stats::lm(mpg ~ disp + hp , data = dados) #removendo peso do modelo
...
abline(0,0)
dev.off()
#nota-se que, de forma geral, os gráficos de dispersão estão bem ajustados, nao existe um padrão na distribuição nos dados,
...
#4º grafico mostra os pontos de alavanca, se os pontos estão fora da linha de confiança, estão dentro
